// ==UserScript==
// @name         padrao_zebra_baixa_2_torresBet365
    ...
    function checkForNewColumn() {
      ...
      for (let i = cells.length - 2; i >= 0; i--) {
       ...
       // Iterar sobre os valores da célula atual
        ...
        let hasValueLessThanThree = Array.from(intersectingValues).some(value => parseFloat(value) <= 3);

        // Verifica se há pelo menos um valor maior ou igual a 5 no conjunto
        let hasValueGreaterOrEqualFive = Array.from(intersectingValues).some(value => parseFloat(value) >= 5);
        // Verifica se há mais de 2 valores no array intersectingValues e se os valores são diferentes
        if (intersectingValues.size >= 2 && hasValueLessThanThree && hasValueGreaterOrEqualFive) {
            let nova_mensagem = `Padrão Zebra Baixa 2 torres: ${currentValues} e ${nextValues}`
